...
import { Crown, Check, Zap, Star, ShieldCheck, Gem, MessageCircle, Ticket } from 'lucide-react';
// Import data voucher
...
export default function Vip() {
  ...
  // SESSION_KEY harus sama dengan di Watch.tsx Boskuh!
  const SESSION_KEY = '_app_session_v2';

  // Sinkronisasi status member saat halaman dibuka
  useEffect(() => {
    const saved = localStorage.getItem(SESSION_KEY);
    if (saved) {
      try {
        const decoded = JSON.parse(atob(saved));
        // Cari data terbaru dari JSON berdasarkan coupon
  ...
  const handleRedeem = () => {
    ...
    if (found) {
      ...
      if (expiryDate > today) {
        // HITUNG SELISIH HARI SESUAI REQUEST BOSKUH
        const diffTime = Math.abs(expiryDate.getTime() - today.getTime());
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

        // SIMPAN DATA (Pakai format yang sama dengan Watch.tsx)
        const secureData = { 
          uid: found.coupon, 
          exp: found.expired,
    ...
    <div className="pb-32 pt-6 px-4 bg-[#050505] min-h-screen text-zinc-100 font-sans">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-zinc-900 to-black border border-white/5 rounded-[2.5rem] p-8 mb-8 shadow-2xl">
        ...
        <div className="mt-8 pt-6 border-t border-white/5 relative z-10 flex justify-between items-end">
          <div>
            <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest mb-1.5 opacity-50">
              Subscription Period
            </p>
            <p className="text-sm font-black text-white uppercase tracking-tighter italic">
              {userData?.isVip 
                ? `Ends: ${userData.expired}` 
                : 'Please activate your VIP'}
            </p>
          </div>
      ...
      {!userData?.isVip && (
        <div className="mb-12 group">
          <div className="bg-zinc-900/30 border border-white/5 rounded-[2rem] p-2 flex gap-2 backdrop-blur-md focus-within:border-yellow-500/50 transition-all">
            <input 
              type="text" 
              placeholder="ENTER VOUCHER CODE"
              value={licenseInput}
              onChange={(e) => setLicenseInput(e.target.value.toUpperCase())}
              className="flex-1 bg-transparent px-5 py-4 text-xs font-black tracking-[0.3em] text-yellow-500 outline-none placeholder:text-zinc-700"
            />
            <button onClick={handleRedeem} className="bg-white text-black px-8 py-4 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all hover:bg-yellow-500">
              Redeem
            </button>
          </div>
        </div>
