...
// IMPORT LANGSUNG DARI FILE LOKAL BOSKUH
...
const Watch = () => {
  ...
  // FUNGSI REDEEM PAKAI JSON LOKAL
  const handleRedeem = () => {
    // Proteksi: Jika sudah berlangganan, blokir redeem lagi
    if (isSubscribed) {
      alert("VIP Kamu masih aktif, Boskuh!");
      return;
    }

    if (!licenseCode.trim()) return alert("Masukkan kode Bos!");
    
    // Cari di file userVip.json
    ...
    if (found) {
      ...
      if (expiry > today) {
        ...
        alert("Waduh, KODE EXPIRED!"); 
      }
    } else { 
      alert("KODE SALAH! Hubungi @leviiwashere"); 
    }
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 pb-32">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6">
                ...
                <div className="rounded-[2rem] overflow-hidden bg-black relative aspect-[9/16] md:aspect-video shadow-2xl">
                  {!isSubscribed && (
                    <div className="absolute inset-0 bg-black/90 backdrop-blur-xl z-40 flex flex-col items-center justify-center p-8 text-center">
                      ...
                      <p className="text-[10px] text-zinc-500 font-bold uppercase mb-8 tracking-[0.2em] leading-relaxed text-balance text-white">
                        Gunakan Voucher VIP untuk menonton episode ini.
                      </p>
            ...
            <div className="flex gap-4">
              <button onClick={() => handleChapterChange(currentChapter - 1)} disabled={currentChapter <= 1} className="w-16 h-16 bg-zinc-900 border border-white/5 rounded-[1.5rem] flex items-center justify-center text-zinc-500 hover:text-white disabled:opacity-10 active:scale-90 transition-all shadow-xl">
                <ChevronLeft size={32} />
              </button>
              <button onClick={() => handleChapterChange(currentChapter + 1)} disabled={currentChapter >= chapters.length} className="flex-1 h-16 relative group overflow-hidden rounded-[1.5rem] active:scale-[0.98] transition-all disabled:opacity-20">
            ...
            <div className="p-8 bg-zinc-900/30 rounded-[2.5rem] border border-white/5">
                ...
                <p className="text-sm text-zinc-400 leading-relaxed font-medium">{watchData.introduction}</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-4">
            <div className="bg-zinc-900/50 p-6 rounded-[2.5rem] border border-white/5 sticky top-24">
               <div className="flex items-center justify-between mb-6 px-2">
                  ...
                  <span className="bg-white/5 px-3 py-1 rounded-full text-[10px] font-black text-zinc-500 uppercase tracking-tighter">{chapters.length} Eps</span>
               </div>
               <div className="grid grid-cols-4 gap-2.5 max-h-[60vh] overflow-y-auto pr-2 no-scrollbar">
                  {chapters.map((chapter) => {
                    ...
                    return (
                      <button key={chapter.chapterId} onClick={() => handleChapterChange(ep)} className={`aspect-square rounded-2xl text-[11px] font-black transition-all flex items-center justify-center relative border-2 ${isActive ? 'bg-red-600 border-red-500 text-white shadow-[0_0_15px_rgba(220,38,38,0.4)] scale-105' : 'bg-black text-zinc-500 border-white/5 hover:border-white/20'}`}>
      ...
      {/* POPUP REDEEM (JSON VERSION) */}
      {showLockPopup && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-2xl flex items-center justify-center z-[200] p-6">
          <div className="bg-zinc-900 border border-white/10 rounded-[3rem] w-full max-w-sm overflow-hidden shadow-[0_0_100px_rgba(0,0,0,1)]">
            ...
            <div className="p-10 text-center">
              <h3 className="text-2xl font-black uppercase tracking-tighter mb-3 italic text-white">VIP Activation</h3>
              <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-[0.2em] mb-10 leading-relaxed">
                Unlock all premium episodes.
              </p>
