...
export const useForYou = (page: number) => {
  const [dramas, setDramas] = useState<Drama[]>([]);
  const [loading, setLoading] = useState(true);
  const { lang } = useLanguage();
  ...
  useEffect(() => {
    const fetchForYou = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/foryou/${page}?lang=${lang}`);
        const data: ApiResponse = await response.json();
        if (data.success) {
          setDramas(data.data.list.slice(0, 6));
        }
      } catch (error) {
...
export const useInfiniteDramas = () => {
  ...
  const fetchDramas = useCallback(async (pageNum: number, isLoadMore = false) => {
    ...
    try {
      ...
      console.error('Failed to fetch infinite dramas:', error);
    } finally {
      setLoading(false);
...
export const useRankDramas = () => {
  ...
  useEffect(() => {
    const fetchDramas = async () => {
      try {
        const response = await fetch(`/api/rank/1?lang=${lang}`);
        const data: ApiResponse = await response.json();
        if (data.success) {
          setDramas(data.data.list);
        }
...
export const useSearchDramas = (query: string) => {
  ...
  useEffect(() => {
    ...
    const fetchDramas = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/search/${encodeURIComponent(query)}/1?lang=${lang}&pageSize=20`);
        const data: ApiResponse = await response.json();
        if (data.success) {
          setDramas(data.data.list);
        }
